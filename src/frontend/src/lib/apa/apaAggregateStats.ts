 ...
 * 
 * @param officialOnly - When true, only includes Official APA match logs (excludes practice matches)
 */
export function extractPlayerApaMatches(matches: ApiMatch[], playerName: string, officialOnly: boolean = false): ApaAggregateDataPoint[] {
  const normalizedPlayerName = normalizePlayerName(playerName);
  const dataPoints: ApaAggregateDataPoint[] = [];

  for (const match of matches) {
    // APA Practice matches - skip if officialOnly is true
    if (!officialOnly && match.mode === MatchMode.apaPractice && match.apaMatchInfo) {
    ...
    // Official APA Match Logs - always include these
...
/**
 * Compute average of last 10 out of 20 matches
 */
export function computeLast10Of20Average(values: (number | null)[]): number | null {
  // Filter out null values
  const validValues = values.filter((v): v is number => v !== null);
  ...
  // Get the 10 most recent from those 20
  const last10 = last20.slice(-10);

  if (last10.length === 0) return null;

  return last10.reduce((sum, val) => sum + val, 0) / last10.length;
}
 ...
 */
export function extractOfficialApaWinRate(matches: ApiMatch[]): {
  total: number;
  wins: number;
  winRate: number | null;
} {
  const officialMatches = matches
    ...
    .sort((a, b) => getEffectiveMatchTimestamp(a) - getEffectiveMatchTimestamp(b));

  const last20 = officialMatches.slice(-20);

  const matchesWithKnownOutcome = last20.filter(m => m.officialApaMatchLogData?.didWin !== undefined);
  const wins = matchesWithKnownOutcome.filter(m => m.officialApaMatchLogData?.didWin === true).length;
  const total = matchesWithKnownOutcome.length;
