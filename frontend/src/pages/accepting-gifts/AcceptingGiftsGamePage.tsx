...
export default function AcceptingGiftsGamePage() {
  ...
  const handleAttemptInput = (value: string) => {
    const attemptValue = parseInt(value) || 0;
    const updatedState = applyAttemptResult(gameState, attemptValue);
    setGameState(updatedState);

    if (updatedState.playerSetScore >= 7 || updatedState.ghostSetScore >= 7) {
  ...
  const handleEndSession = async () => {
    ...
    try {
      const playerWon = gameState.playerSetScore >= 7;
      const { matchId, matchRecord } = buildAcceptingGiftsMatch({
        playerName: gameState.playerName,
        notes: gameState.notes,
        startingObjectBallCount: currentLevel.objectBallCount,
        endingObjectBallCount: currentLevel.objectBallCount,
  ...
  const handleContinueNextSet = () => {
    const nextState = prepareNextSet(gameState);
  ...
  const setComplete = gameState.playerSetScore >= 7 || gameState.ghostSetScore >= 7;
  const playerWon = gameState.playerSetScore >= 7;

  // Build the array of ball numbers to display: 0 through objectBallCount (inclusive)
  // e.g. 2+8 (objectBallCount=2) → [0, 1, 2, 3] where 3 = maxAttempt = runout
  const ballButtons = Array.from({ length: currentLevel.objectBallCount + 2 }, (_, i) => i);
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="text-lg font-semibold">
                {gameState.playerSetScore} - {gameState.ghostSetScore}
              </div>
            </div>
          </div>
        </CardContent>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                Balls Made (0–{currentLevel.objectBallCount})
              </p>
              <div className="flex flex-wrap gap-3">
                {ballButtons.map((ballNum) => (
                  <button
                    key={ballNum}
                    onClick={() => handleAttemptInput(String(ballNum))}
                    className="flex h-12 w-12 items-center justify-center rounded-full border-2 border-border bg-background text-lg font-bold transition-colors hover:bg-primary hover:text-primary-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
                    aria-label={`${ballNum} balls made`}
                  >
                    {ballNum}
                  </button>
                ))}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-3">
              <Button
                onClick={handleSaveAndStartNext}
                size="lg"
                className="w-full bg-green-600 hover:bg-green-700 text-white"
                disabled={saveMatch.isPending || !isAuthenticated}
              >
                {saveMatch.isPending ? 'Saving...' : 'Save & Start Next Match'}
              </Button>
              <Button onClick={handleEndSession} variant="outline" size="lg" className="w-full" disabled={saveMatch.isPending || !isAuthenticated}>
                {saveMatch.isPending ? 'Saving...' : 'Save & Exit'}
              </Button>
              <Button onClick={() => setShowEndDialog(true)} variant="ghost" size="lg" className="w-full">
