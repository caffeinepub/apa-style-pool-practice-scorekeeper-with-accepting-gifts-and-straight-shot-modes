 ...
 * aPPI: For wins only, aPPI = max(PPI, expectedPpiFromTable); losses show --
 */
 ...
 * For losses: aPPI is NOT calculated — returns null (show --)
...
export function computeOfficialApaAppiWithContext(
  ...
  // FIX (REQ-2): For losses or unknown outcomes, aPPI is not shown — return null
