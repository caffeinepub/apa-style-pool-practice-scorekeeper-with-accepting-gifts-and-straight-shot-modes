...
// ─── Navigation Origin Helpers ───────────────────────────────────────────────
// Used to track where the user navigated from so back-links can be context-aware.
...
export function setNavigationOrigin(origin: string): void {
    try {
        sessionStorage.setItem(NAV_ORIGIN_KEY, origin);
    } catch (error) {
        console.warn('Failed to set navigation origin:', error);
    }
}

/**
 * Retrieves the stored navigation origin.
 *
 * @returns The origin string if set, null otherwise
 */
export function getNavigationOrigin(): string | null {
    try {
        return sessionStorage.getItem(NAV_ORIGIN_KEY);
    } catch (error) {
        console.warn('Failed to get navigation origin:', error);
        return null;
    }
}
...
export function clearNavigationOrigin(): void {
    try {
        ...
        console.warn('Failed to clear navigation origin:', error);
    }
}
